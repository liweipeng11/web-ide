...
test("plan mode exposes only readonly tools to the model", async () => {
  ...
  assert.equal(result.status, "completed");
  assert.deepEqual(toolNames.sort(), ["checkExistence", "findSimilarPatterns", "inspectProject", "listCodeDefinitionNames", "listFiles", "readFile", "readFileChunk", "readFileRange", "searchCode", "searchCodeRegex", "searchFilesByName"].sort());
});
