...
test("阶段 5 验收覆盖默认启用、显式回退和十类集成场景", async () => {
  ...
  assert.equal(report.checks.filter((item) => item.category === "default_activation").length, 4);
  assert.equal(report.checks.filter((item) => item.category === "rollback").length, 4);
...
test("完成度低于 90% 时拒绝通过阶段 5 验收", async () => {
  ...
  const failedCaseCount = 2;
  const failedEvaluation = {
    ...evaluation,
    summary: { ...evaluation.summary, passed: evaluation.summary.total - failedCaseCount, failed: failedCaseCount, successRate: 80 },
